  ...
  <el-row :gutter="20">
    ...
    <el-col :xs="24" :sm="5">
      <el-card style="background-color: rgba(255,255,255,0.9)" class=" right-item">
        ...
        <ul class=" blog-type-ul" style="margin-top: 5px;">
          <li class=" blog-type-li" v-for="(cmsType,index) in typeList" :key="cmsType.typeId" @click="selectType(cmsType)"
              :class="cmsType.typeId === typeId? 'activeType':''" :style="{ backgroundColor: index % 2 == 1 ? '#aaeaaa' : '#ff4141' }">
    ...
    <el-col :xs="24" :sm="15">
      <el-card style="background-color: rgba(255,255,255,0.9)" class="left-item">
        <ul>
        <li type="flex" align="middle" style="flex-wrap: wrap" :gutter="20" v-for="(blog,index) in blogList" :key="blog.id"
          shadow="never" class="blog-type-li" :style="{ backgroundColor: index % 2 == 1 ? '#aaeaaa' : '#ff4141' }">
...
<style scoped>
  ...
  @media screen and (max-width: 768px) {
    ...
    .even-row {
      background-color: #c24a4a !important; /* 双数行背景色 */
    }

    .odd-row {
      background-color: #fff !important; /* 单数行背景色 */
    }
  }
</style>
